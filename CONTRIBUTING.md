...
## Maintainer automation setup

The weekly docs sync workflows open pull requests when the upstream Rive docs structure or monitored page content changes. Configure a repository secret named `RIVE_ASSISTANT_BOT_TOKEN` with a fine-grained GitHub token that has access to this repository and these permissions:

- Contents: read and write
- Pull requests: read and write

The workflows intentionally use this token instead of `GITHUB_TOKEN` so branch-protection checks run normally on automation-created pull requests.
